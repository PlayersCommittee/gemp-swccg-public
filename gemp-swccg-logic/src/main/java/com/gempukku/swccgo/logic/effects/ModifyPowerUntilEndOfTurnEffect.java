...
/**
 * An effect to modify the power of card until end of the turn.
 */
public class ModifyPowerUntilEndOfTurnEffect extends AbstractSuccessfulEffect {
    private PhysicalCard _cardToModify;
    private float _modifierAmount;
    private boolean _cumulatively;

    /**
     * Creates an effect that modifies the power of a card until end of the turn.
     * @param action the action performing this effect
     * @param cardToModify the card whose power is modified
     * @param modifierAmount the amount of the modifier
     */
    public ModifyPowerUntilEndOfTurnEffect(Action action, PhysicalCard cardToModify, float modifierAmount) {
     ...
     * @param cardToModify the card whose power is modified
     * @param modifierAmount the amount of the modifier
     * @param cumulatively true if cumulatively, otherwise false
     */
    public ModifyPowerUntilEndOfTurnEffect(Action action, PhysicalCard cardToModify, float modifierAmount, boolean cumulatively) {
        super(action);
        _cardToModify = cardToModify;
        _modifierAmount = modifierAmount;
        _cumulatively = cumulatively;
    }
    ...
    protected void doPlayEffect(SwccgGame game) {
        ...
        if (_modifierAmount < 0)
            actionMsg = "reduces " + GameUtils.getCardLink(_cardToModify) + "'s power by " + GuiUtils.formatAsString(-_modifierAmount) + " until end of the turn";
        else
            actionMsg = "adds " + GuiUtils.formatAsString(_modifierAmount) + " to " + GameUtils.getCardLink(_cardToModify) + "'s power until end of the turn";
