...
/**
 * The game process for the Activate phase.
 */
public class ActivatePhaseGameProcess implements GameProcess {
    private SwccgGame _game;

    @Override
    public void process(SwccgGame game) {
        _game = game;
        game.getGameState().setCurrentPhase(Phase.ACTIVATE);

        // Take game snapshot for start of phase
        game.takeSnapshot("Start of " + game.getGameState().getCurrentPlayerId() + "'s " + Phase.ACTIVATE.getHumanReadable().toLowerCase() + " phase #" + game.getGameState().getPlayersLatestTurnNumber(game.getGameState().getCurrentPlayerId()));
    }

    @Override
    public GameProcess getNextProcess() {
        return new StartOfPhaseGameProcess(Phase.ACTIVATE,
                new PlayersPlayPhaseActionsInOrderGameProcess(_game.getGameState().getPlayerOrder().getPlayOrder(_game.getGameState().getCurrentPlayerId(), true), 0,
                        new EndOfPhaseGameProcess(Phase.ACTIVATE,
                                new ControlPhaseGameProcess())));
    }
}
