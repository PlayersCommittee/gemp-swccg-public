...
/*
    ...
    /**
     * Builds a fire weapon action for Disruptor Pistol (V).
     * @return the action
     */
    public FireSingleWeaponAction buildDisruptorPistolVAction() {
        final FireSingleWeaponAction action = new FireSingleWeaponAction(_sourceCard, _weaponOrCardWithPermanentWeapon, _permanentWeapon, _repeatedFiring, _targetedAsCharacter, _defenseValueAsCharacter, _fireAtTargetFilter, _ignorePerAttackOrBattleLimit);
        action.setText("Fire " + action.getWeaponTitle(_game));
                                        ...
                                        action.appendEffect(
                                                new DrawDestinyEffect(action, _playerId, 1, DestinyType.WEAPON_DESTINY) {
                                                    ...
                                                    protected void destinyDraws(SwccgGame game, List<PhysicalCard> destinyCardDraws, List<Float> destinyDrawValues, Float totalDestiny) {
                                                        ...
                                                        // Compares to *printed* defense value
                                                        float valueToCompare = 0;
                                                        float armor = 0;
                                                        float maneuver = 0;
                                                        float ability = 0;

                                                        if (cardFiredAt.getBlueprint().hasArmorAttribute() && cardFiredAt.getBlueprint().getArmor() != null) {
                                                            armor = cardFiredAt.getBlueprint().getArmor();
                                                        }
                                                        if (cardFiredAt.getBlueprint().hasManeuverAttribute() && cardFiredAt.getBlueprint().getManeuver() != null) {
                                                            maneuver = cardFiredAt.getBlueprint().getManeuver();
                                                        }
                                                        if (cardFiredAt.getBlueprint().hasAbilityAttribute() && cardFiredAt.getBlueprint().getAbility() != null) {
                                                            armor = cardFiredAt.getBlueprint().getAbility();
                                                        }

                                                        valueToCompare = Math.max(valueToCompare, armor);
                                                        valueToCompare = Math.max(valueToCompare, maneuver);
                                                        valueToCompare = Math.max(valueToCompare, ability);

                                                        gameState.sendMessage("Printed Defense value: " + GuiUtils.formatAsString(valueToCompare));
