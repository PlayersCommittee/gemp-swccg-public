...
/**
 * An effect to exchange cards from hand with top card from the specified card pile.
 */
public class ExchangeCardsInHandWithTopCardInCardPileEffect extends AbstractSubActionEffect {
    ...
    /**
     * Creates an effect to exchange cards from hand with top card from the specified card pile.
     ...
     */
    protected ExchangeCardsInHandWithTopCardInCardPileEffect(Action action, String playerId, Zone cardPile, int minimum, int maximum) {
        super(action);
        _playerId = playerId;
        _cardPile = cardPile;
        _minimum = minimum;
     ...
     */
    protected ExchangeCardsInHandWithTopCardInCardPileEffect(Action action, String playerId, Zone cardPile, int minimum, int maximum, Filterable cardsInHandFilter) {
        super(action);
        _playerId = playerId;
        _cardPile = cardPile;
        _minimum = minimum;
    ...
    protected SubAction getSubAction(final SwccgGame game) {
        ...
        final PhysicalCard cardFromPile = gameState.getTopOfCardPile(_playerId, _cardPile);
                    ...
                    new PassthruEffect(subAction) {
                        ...
                        protected void doPlayEffect(SwccgGame game) {
                            if (!_cardsToPlaceInCardPile.isEmpty()) {
                                if (_hidden) {
                                    gameState.sendMessage(_playerId + " exchanges " + GameUtils.numCards(_cardsToPlaceInCardPile) + " from hand with top card in " + _cardPile.getHumanReadable());
                                }
                                else {
                                    gameState.sendMessage(_playerId + " exchanges " + GameUtils.getAppendedNames(_cardsToPlaceInCardPile) + " from hand with " + GameUtils.getCardLink(cardFromPile) + " from top of " + _cardPile.getHumanReadable());
                                }
