...
/**
 * The effect result that is emitted when a card begins to move using hyperspeed.
 */
public class MovingUsingHyperspeedResult extends EffectResult implements MovingResult {
    ...
    /**
     * Creates an effect result that is emitted when a card begins to move using hyperspeed.
     ...
     */
    public MovingUsingHyperspeedResult(PhysicalCard cardMoving, String playerId, PhysicalCard movingFrom, PhysicalCard movingTo, boolean asReact, boolean asMoveAway, PreventableCardEffect effect) {
        super(Type.MOVING_USING_HYPERSPEED, playerId);
    ...
    public String getText(SwccgGame game) {
        return "Moving " + (_asMoveAway ? "away " : "") + GameUtils.getCardLink(_cardMoving) + " from " + GameUtils.getCardLink(_movingFrom) + " to " + GameUtils.getCardLink(_movingTo) + " using hyperspeed" + (_asReact ? " as a 'react'" : "");
    }
}
