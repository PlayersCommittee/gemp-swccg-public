...
/**
 * The effect result that is emitted when a card disembarks from a card (or from a location).
 */
public class DisembarkedResult extends EffectResult implements MovedResult {
    private PhysicalCard _cardMoved;
    private PhysicalCard _movedFrom;
    private PhysicalCard _movedTo;

    /**
     * Creates an effect result that is emitted when a card disembarks from a card (or from a location).
     * @param movedCard the card that moved
     * @param playerId the performing player
     * @param moveFrom the card moved from
     * @param movedTo the card disembarked to
     */
    public DisembarkedResult(PhysicalCard movedCard, String playerId, PhysicalCard moveFrom, PhysicalCard movedTo) {
        super(Type.DISEMBARKED, playerId);
    ...
    public String getText(SwccgGame game) {
        return "Disembarked " + GameUtils.getCardLink(_cardMoved) + " from " + GameUtils.getCardLink(_movedFrom) + " to " + GameUtils.getCardLink(_movedTo);
    }
}
