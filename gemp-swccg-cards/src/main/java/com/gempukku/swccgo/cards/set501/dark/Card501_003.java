...
public class Card501_003 extends AbstractNormalEffect {
    public Card501_003() {
        ...
        setGameText("Deploy on table. [Episode VII] troopers are forfeit +1 (+2 if non-unique). If [Episode VII] Emperor on table, once per turn may deploy Kijimi from Reserve Deck (reshuffle) or place any three cards of your Lost Pile out of play to deploy an [Episode VII] trooper from your Lost Pile. [Immune to Alter.]");
    ...
    protected List<Modifier> getGameTextWhileActiveInPlayModifiers(SwccgGame game, PhysicalCard self) {
        ...
        modifiers.add(new ForfeitModifier(self, Filters.and(Filters.icon(Icon.EPISODE_VII), Filters.trooper), new CardMatchesEvaluator(1, 2, Filters.non_unique)));
        return modifiers;
    }
    ...
    protected List<TopLevelGameTextAction> getGameTextTopLevelActions(final String playerId, SwccgGame game, final PhysicalCard self, int gameTextSourceCardId) {
        ...
        if (GameConditions.canSpot(game, self, Filters.and(Filters.icon(Icon.EPISODE_VII), Filters.Emperor))
            ...
            if (GameConditions.canDeployCardFromLostPile(game, playerId, self, gameTextActionId)
                ...
                // Perform result(s)
                action.appendEffect(
                        new DeployCardFromLostPileEffect(action, Filters.and(Filters.icon(Icon.EPISODE_VII), Filters.trooper), false));
