...
/**
 * An effect that pays the cost for a starship or vehicle to take off.
 */
public class PayTakeOffCostEffect extends AbstractSubActionEffect {
    ...
    /**
     * Creates an effect that pays the cost for a starship or vehicle to take off.
     ...
     */
    public PayTakeOffCostEffect(Action action, String playerId, PhysicalCard cardToMove, PhysicalCard moveTo, boolean asReact, float changeInCost) {
        super(action);
        _playerId = playerId;
    ...
    protected SubAction getSubAction(SwccgGame game) {
        ...
        float moveCost = game.getModifiersQuerying().getTakeOffCost(game.getGameState(), _cardToMove, _moveFrom, _moveTo, _asReact, _changeInCost);
        if (moveCost > 0) {
