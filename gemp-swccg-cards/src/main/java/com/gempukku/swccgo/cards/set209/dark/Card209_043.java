...
public class Card209_043 extends AbstractNormalEffect {
    public Card209_043() {
        ...
        setLore("");
        setGameText("If Agents Of Black Sun on table, deploy on table. Once per turn, may use 1 Force to [download] a Coruscant site. Once per turn, if Emperor on Coruscant, may draw top card of Force Pile (if you occupy three battlegrounds, opponent also loses 1 Force). [Immune to Alter.]");
    ...
    protected List<TopLevelGameTextAction> getGameTextTopLevelActions(final String playerId, SwccgGame game, final PhysicalCard self, int gameTextSourceCardId) {
        ...
        gameTextActionId = GameTextActionId.SHADOWS_OF_THE_EMPIRE__DOWNLOAD_CORUSCANT_SITE;
        if (GameConditions.isOncePerTurn(game, self, playerId, gameTextSourceCardId, gameTextActionId)
                && GameConditions.canUseForce(game, playerId, 1)
                && GameConditions.canDeployCardFromReserveDeck(game, playerId, self, gameTextActionId)) {

            final TopLevelGameTextAction action = new TopLevelGameTextAction(self, gameTextSourceCardId, gameTextActionId);
            action.setText("Deploy Coruscant site from Reserve Deck");
            action.setActionMsg("Deploy a Coruscant site from Reserve Deck");
            ...
            // Perform result(s)
            action.appendEffect(
                    new DeployCardFromReserveDeckEffect(action, Filters.Coruscant_site, true));
            actions.add(action);
        }
