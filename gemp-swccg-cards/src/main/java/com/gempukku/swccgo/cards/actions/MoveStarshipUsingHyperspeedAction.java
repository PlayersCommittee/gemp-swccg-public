...
/**
 * An action to move a starship using hyperspeed.
 */
public class MoveStarshipUsingHyperspeedAction extends AbstractTopLevelRuleAction {
    ...
    /**
     * Creates an action to move a starship using hyperspeed.
     ...
     */
    public MoveStarshipUsingHyperspeedAction(final String playerId, final PhysicalCard card, boolean forFree, boolean asReact, boolean asMoveAway, final Filter moveTargetFilter) {
        super(card, playerId);
        _cardToMove = card;
        _forFree = forFree;
        _asReact = asReact;
        ...
        appendTargeting(
                new ChooseCardOnTableEffect(_that, playerId, "Choose where to move " + GameUtils.getCardLink(card) + " using hyperspeed", moveTargetFilter) {
                    @Override
                    protected void cardSelected(PhysicalCard selectedCard) {
                        _systemToMoveTo = selectedCard;

                        _moveCardEffect = new MoveStarshipUsingHyperspeedEffect(_that, card, _systemToMoveTo, _asReact, _asMoveAway);
                    }
                }
        );
    }
        ...
        if (!isAnyCostFailed()) {
            ...
            if (!_useForceCostApplied) {
                ...
                if (!_forFree && !_asReact) {
                    appendCost(new PayMoveUsingHyperspeedCostEffect(_that, getPerformingPlayer(), _cardToMove, _systemToMoveTo, false, 0));
                    return getNextCost();
                }
            }
