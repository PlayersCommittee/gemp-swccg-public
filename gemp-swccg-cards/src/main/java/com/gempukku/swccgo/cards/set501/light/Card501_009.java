...
public class Card501_009 extends AbstractNormalEffect {
    public Card501_009() {
        ...
        setGameText("Deploy on table. [DSII] Jedi are lost. Qui-Gon is deploy -1. Unless with Anakin or Vader, Mace is immune to attrition. Once per turn, may choose: deploy an [Episode I] lightsaber from Reserve Deck (reshuffle), or once per game, deploy Malastare, Mos Espa, or Night Club from Reserve Deck; reshuffle. (Immune to Alter)");
        addIcons(Icon.EPISODE_I, Icon.VIRTUAL_SET_10);
        addImmuneToCardTitle(Title.Alter);
        setTestingText("Jedi Business (Errata)");
    }
    ...
    protected List<RequiredGameTextTriggerAction> getGameTextRequiredAfterTriggers(SwccgGame game, EffectResult effectResult, PhysicalCard self, int gameTextSourceCardId) {
        ...
        if (TriggerConditions.isTableChanged(game, effectResult)) {
            Collection<PhysicalCard> dsIIJedi = Filters.filterActive(game, self, Filters.and(Filters.Jedi, Icon.DEATH_STAR_II));
            if (!dsIIJedi.isEmpty()) {

                final RequiredGameTextTriggerAction action = new RequiredGameTextTriggerAction(self, gameTextSourceCardId);
                action.setSingletonTrigger(true);
                action.setText("Make [DSII] Jedi lost");
                action.setActionMsg("Make " + GameUtils.getAppendedNames(dsIIJedi) + " lost");

                // Perform result(s)
                action.appendEffect(
                        new LoseCardsFromTableEffect(action, dsIIJedi));
                actions.add(action);
            }
        }
        return actions;
