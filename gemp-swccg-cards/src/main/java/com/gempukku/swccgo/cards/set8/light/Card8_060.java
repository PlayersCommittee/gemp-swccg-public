...
public class Card8_060 extends AbstractLostInterrupt {
    ...
    protected List<PlayInterruptAction> getGameTextOptionalAfterActions(final String playerId, final SwccgGame game, final EffectResult effectResult, final PhysicalCard self) {
        ...
        // Check condition(s)
        if (TriggerConditions.forceDrainInitiatedBy(game, effectResult, opponent, Filters.exterior_Endor_site)
            ...
            action.allowResponses(
                    new RespondablePlayCardEffect(action) {
                        ...
                        protected void performActionResults(Action targetingAction) {
                            ...
                            action.appendEffect(
                                    new DeployCardsToLocationFromReserveDeckEffect(action, Filters.and(Filters.non_unique, Filters.Ewok), 0, Integer.MAX_VALUE, forceDrainLocation, true, true) {
                                        public String getChoiceText(int numCardsToChoose) {
                                            return "Choose non-unique Ewoks to deploy from Reserve Deck";
                                        }
