...
public class Card13_073_BACK extends AbstractObjective {
    ...
    protected List<Modifier> getGameTextWhileActiveInPlayModifiers(SwccgGame game, PhysicalCard self) {
        ...
        Filter sameLocationAsYourDarkJedi = Filters.sameLocationAs(self, Filters.and(Filters.your(self), Filters.Dark_Jedi));

        List<Modifier> modifiers = new LinkedList<Modifier>();
        modifiers.add(new MayNotForceDrainAtLocationModifier(self, sameLocationAsYourDarkJedi, playerId));
        modifiers.add(new MayNotInitiateBattleAtLocationModifier(self, sameLocationAsYourDarkJedi, playerId));
        return modifiers;
    }
    ...
    protected List<TopLevelGameTextAction> getGameTextTopLevelActions(final String playerId, SwccgGame game, final PhysicalCard self, int gameTextSourceCardId) {
        ...
        Filter darkJediFilter = Filters.and(Filters.your(self), Filters.Dark_Jedi, Filters.at(Filters.wherePresent(self, Filters.and(Filters.opponents(self), Filters.Jedi, Filters.canBeTargetedBy(self)))));
            ...
            action.appendTargeting(
                    new TargetCardOnTableEffect(action, playerId, "Choose Dark Jedi", darkJediFilter) {
                        ...
                        protected void cardTargeted(final int targetGroupId, final PhysicalCard darkJedi) {
                            Filter jediFilter = Filters.and(Filters.opponents(self), Filters.Jedi, Filters.present(darkJedi));
                            action.appendTargeting(
                                    new TargetCardOnTableEffect(action, playerId, "Choose Jedi", jediFilter) {
                                        ...
                                        protected void cardTargeted(final int targetGroupId, final PhysicalCard jedi) {
                                            action.addAnimationGroup(darkJedi, jedi);
                                            // Allow response(s)
