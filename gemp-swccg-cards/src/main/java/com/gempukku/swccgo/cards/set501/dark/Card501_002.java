...
public class Card501_002 extends AbstractAlien {
    ...
    protected List<Modifier> getGameTextWhileActiveInPlayModifiers(SwccgGame game, final PhysicalCard self) {
        ...
        modifiers.add(new TotalBattleDestinyModifier(self, Filters.here(self), new OrCondition(new ArmedWithCondition(self, Filters.any), new WithCondition(self, Filters.captive)), 1, self.getOwner()));
        return modifiers;
    }
    ...
    protected List<OptionalGameTextTriggerAction> getGameTextOptionalAfterTriggers(final String playerId, SwccgGame game, EffectResult effectResult, PhysicalCard self, int gameTextSourceCardId) {
        ...
        if (TriggerConditions.justDeployed(game, effectResult, self.getOwner(), self)
            ...
            int numCardsToPeakAt = GameConditions.isHere(game, self, Filters.captive) ? 6 : 4;

            final OptionalGameTextTriggerAction action = new OptionalGameTextTriggerAction(self, gameTextSourceCardId);
            action.setText("Peak at " + numCardsToPeakAt + " cards");
            action.setActionMsg("Peak at " + numCardsToPeakAt +  " cards");
            // Perform result(s)
            action.appendEffect(
                    new PeekAtTopCardsOfCardPileEffect(action, playerId, playerId, Zone.USED_PILE, numCardsToPeakAt){
                        @Override
                        protected void cardsPeekedAt(List<PhysicalCard> peekedAtCards) {
                           new TakeCardIntoHandFromUsedPileEffect(action, playerId, Filters.in(peekedAtCards),true);
                        }
                    });
