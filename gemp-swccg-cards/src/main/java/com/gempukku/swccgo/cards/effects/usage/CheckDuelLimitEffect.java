...
/**
 * An effect that checks if a game text usage limit per duel has been reached. The effect will be successful if the limit
 * had not yet been reached.
 */
class CheckDuelLimitEffect extends AbstractUsageLimitEffect {
    ...
    /**
     * Creates an effect that checks if a game text usage limit per duel for an action of the specified card has been
     * reached.
     * @param action the action performing this effect
     * @param limit the limit
     */
    protected CheckDuelLimitEffect(GameTextAction action, int limit) {
        super(action);
    ...
    public FullEffectResult playEffectReturningResult(SwccgGame game) {
        int incrementedBy = game.getModifiersQuerying().getUntilEndOfDuelLimitCounter(_card, _playerId, _gameTextSourceCardId, _gameTextActionId).incrementToLimit(_limit, 1);
