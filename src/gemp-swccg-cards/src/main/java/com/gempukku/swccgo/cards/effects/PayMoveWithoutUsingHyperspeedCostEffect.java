...
/**
 * An effect that pays the cost of moving a card between systems without using hyperspeed.
 */
public class PayMoveWithoutUsingHyperspeedCostEffect extends AbstractSubActionEffect {
     ...
     * @param moveTo the system the card is moving to
     * @param asReact true if the movement is for a 'react' movement, otherwise false
     * @param changeInCost change in amount of Force (can be positive or negative) required
     */
    public PayMoveWithoutUsingHyperspeedCostEffect(Action action, String playerId, PhysicalCard cardToMove, PhysicalCard moveTo, boolean asReact, float changeInCost) {
        super(action);
        _playerId = playerId;
    ...
    protected SubAction getSubAction(SwccgGame game) {
        ...
        float moveCost = game.getModifiersQuerying().getMoveWithoutUsingHyperspeedCost(game.getGameState(), _cardToMove, _moveFrom, _moveTo, _asReact, _changeInCost);
        if (moveCost > 0) {
