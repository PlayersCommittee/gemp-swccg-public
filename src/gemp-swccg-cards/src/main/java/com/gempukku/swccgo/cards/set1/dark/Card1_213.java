 ...
 */
public class Card1_213 extends AbstractNormalEffect {
    public Card1_213() {
        super(Side.DARK, 3, PlayCardZoneOption.ATTACHED, Title.Death_Star_Sentry, Uniqueness.UNIQUE, ExpansionSet.PREMIERE, Rarity.U1);
        ...
        setGameText("Use 2 Force to deploy on any Death Star site. Declare one of the following to affect that site and adjacent sites while 'sentry' present: Opponent's deploy +1. OR Opponent's ability required for battle destiny +1. OR Your total power +1.");
    ...
    protected Filter getGameTextValidDeployTargetFilter(SwccgGame game, PhysicalCard self, PlayCardOptionId playCardOptionId, boolean asReact) {
        return Filters.Death_Star_site;
    }
    ...
    protected List<Modifier> getGameTextWhileActiveInPlayModifiers(SwccgGame game, final PhysicalCard self) {
        ...
        Condition appliesAllModifiers = new GameTextModificationCondition(self, ModifyGameTextType.DEATH_STAR_SENTRY__APPLIES_ALL_MODIFIERS);
    ...
    protected List<RequiredGameTextTriggerAction> getGameTextRequiredAfterTriggers(SwccgGame game, EffectResult effectResult, PhysicalCard self, int gameTextSourceCardId) {
        if (TriggerConditions.isTableChanged(game, effectResult)
                && GameConditions.canSpot(game, self, 2, Filters.and(Filters.Death_Star_Sentry, Filters.unique))) {

            final RequiredGameTextTriggerAction action = new RequiredGameTextTriggerAction(self, gameTextSourceCardId);

            Collection<PhysicalCard> deathStarSentries = Filters.filterActive(game, self, Filters.Death_Star_Sentry);
            PhysicalCard firstDeathStarSentries = Filters.findFirstActive(game, self, Filters.Death_Star_Sentry);
            deathStarSentries.remove(firstDeathStarSentries);
            action.appendEffect(
                    new PlaceCardsInUsedPileFromTableEffect(action, self.getOwner(), deathStarSentries)
            );
