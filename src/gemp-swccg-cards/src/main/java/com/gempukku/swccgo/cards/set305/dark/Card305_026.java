 ...
 * Type: Weapon
 * Subtype: Character
 * Title: Delak Krennel's Lightsaber
 */
public class Card305_026 extends AbstractCharacterWeapon {
    public Card305_026() {
        super(Side.DARK, 1, "Delak Krennel's Lightsaber", Uniqueness.UNIQUE, ExpansionSet.ABT, Rarity.C);
        setLore("Custom-built by Delak Krennel. Called 'Shiny and Chrome' by Delak.");
        setGameText("Deploy on Delak. May add 1 to Force drain where present. May target a character or creature for free. Draw two destiny. Target hit, and its forfeit = 0, if total destiny > defense value.");
        addPersona(Persona.DELAKS_LIGHTSABER);
        addKeywords(Keyword.LIGHTSABER);
        setMatchingCharacterFilter(Filters.Delak);
    }

    @Override
    protected Filter getGameTextValidDeployTargetFilter(SwccgGame game, PhysicalCard self, PlayCardOptionId playCardOptionId, boolean asReact) {
        return Filters.and(Filters.your(self), Filters.Delak);
    }

    @Override
    protected Filter getGameTextValidToUseWeaponFilter(final SwccgGame game, final PhysicalCard self) {
        return Filters.Delak;
    }
