...
/**
 * An effect that performs a move away as a 'react'.
 */
public class MoveAwayAsReactEffect extends AbstractSubActionEffect {
    private PhysicalCard _cardToMove;
    private boolean _forFree;

    /**
     * Creates an effect that performs a move away as a 'react'.
     * @param action the action performing this effect
     * @param cardToMove the card to move as a 'react'
     * @param forFree true if the movement is to be free, otherwise false
     */
    public MoveAwayAsReactEffect(Action action, PhysicalCard cardToMove, boolean forFree) {
        super(action);
        ...
        subAction.appendEffect(
                new PassthruEffect(subAction) {
                    ...
                    protected void doPlayEffect(SwccgGame game) {
                        ReactActionOption reactActionOption = new ReactActionOption(_action.getActionSource(), _forFree, 0, true, "Move away as a 'react'", _cardToMove, Filters.any, null, false);
