 ...
 * occupies with a card accepted by the specified filter.
 */
public class OccupiesWithEvaluator extends BaseEvaluator {
     ...
     * player occupies with a card accepted by the specified filter.
     * @param playerId the player
     * @param locationFilter the location filter
     * @param withFilter the filter for cards to occupy with
     */
    public OccupiesWithEvaluator(PhysicalCard source, String playerId, Filterable locationFilter, Filterable withFilter) {
    ...
    public float evaluateExpression(GameState gameState, ModifiersQuerying modifiersQuerying, PhysicalCard self) {
        ...
        return Filters.countTopLocationsOnTable(gameState.getGame(), Filters.and(_locationFilter, Filters.occupiesWith(_playerId, source, filterToUse)));
    }
}
