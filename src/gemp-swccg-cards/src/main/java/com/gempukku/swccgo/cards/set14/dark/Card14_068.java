 ...
 */
public class Card14_068 extends AbstractAdmiralsOrder {
    public Card14_068() {
        super(Side.DARK, "The Deflector Shield Is Too Strong", ExpansionSet.THEED_PALACE, Rarity.R);
    ...
    protected List<TopLevelGameTextAction> getGameTextTopLevelActions(final String playerId, SwccgGame game, final PhysicalCard self, int gameTextSourceCardId) {
        ...
        if (GameConditions.isOnceDuringYourPhase(game, self, playerId, gameTextSourceCardId, Phase.CONTROL)) {
            ...
            final Filter yourDroidStarfighter = Filters.and(Filters.your(self), Filters.piloted, Filters.droid_starfighter, Filters.presentWith(self, opponentsStarfighter));
            if (GameConditions.canTarget(game, self, yourDroidStarfighter)) {

                final TopLevelGameTextAction action = new TopLevelGameTextAction(self, gameTextSourceCardId);
                action.setText("Target droid starfighter and opponent's starfighter");
                // Update usage limit(s)
                action.appendUsage(
                        new OncePerPhaseEffect(action));
                // Choose target(s)
                action.appendTargeting(
                        new TargetCardOnTableEffect(action, playerId, "Choose droid starfighter", yourDroidStarfighter) {
                            @Override
