...
import com.gempukku.swccgo.cards.effects.PayEmbarkingCostEffect;
...
import com.gempukku.swccgo.logic.effects.EmbarkEffect;
...
/**
 * An action to embark on a card.
 */
public class EmbarkAction extends AbstractTopLevelRuleAction {
    ...
    private Effect _embarkEffect;
    private boolean _cardMoved;
    private Action _that;

    /**
     * Creates an action to embark on a card.
     ...
     */
    public EmbarkAction(final String playerId, final SwccgGame game, final PhysicalCard card, boolean forFree, final Filter moveTargetFilter) {
        super(card, playerId);
        _cardToMove = card;
        _forFree = forFree;
        _that = this;
        ...
        // Choose card to embark on
        _chooseDestinationEffect =
                new ChooseCardOnTableEffect(_that, playerId, "Choose where to embark " + GameUtils.getCardLink(card), moveTargetFilter) {
                    @Override
                    protected void cardSelected(PhysicalCard selectedCard) {
                        _destination = selectedCard;

                        // Check if embarking to location
                        if (_destination.getBlueprint().getCardCategory() == CardCategory.LOCATION) {
                            _capacitySlotChosen = true;
                            _embarkEffect = new EmbarkEffect(_that, _cardToMove, _destination, false, false);
                        } else {
                            if (_cardToMove.getBlueprint().getCardCategory() == CardCategory.CHARACTER) {
                                    ...
                                    _chooseCapacitySlotEffect = new PlayoutDecisionEffect(_that, getPerformingPlayer(),
                                            new MultipleChoiceAwaitingDecision("Choose capacity slot for  " + GameUtils.getCardLink(_cardToMove) + " aboard " + GameUtils.getCardLink(_destination), seatChoices) {
                                                ...
                                                protected void validDecisionMade(int index, String result) {
                                                    ...
                                                    _embarkEffect = new EmbarkEffect(_that, _cardToMove, _destination, _moveAsPilot, false);
                                                }
                                            });
                                } else {
                                    _capacitySlotChosen = true;
                                ...
                                if (canGoInCapitalStarshipSlot) {
                                    ...
                                    boolean canGoInStarfighterOrTIESlot = Filters.hasAvailableStarfighterOrTIECapacity(_cardToMove).accepts(gameState, modifiersQuerying, _destination);

                                    if (canGoInVehicleSlot && canGoInStarfighterOrTIESlot) {
                                        // Ask player to choose vehicle or starfighter capacity slot
                                        _chooseCapacitySlotEffect = new PlayoutDecisionEffect(_that, getPerformingPlayer(),
                                                new MultipleChoiceAwaitingDecision("Choose capacity slot for  " + GameUtils.getCardLink(_cardToMove) + " in cargo bay of " + GameUtils.getCardLink(_destination), new String[]{"Vehicle", "Starfighter"}) {
                                                    @Override
                                                    protected void validDecisionMade(int index, String result) {
                                                        _capacitySlotChosen = true;
                                                        ...
                                                        _embarkEffect = new EmbarkEffect(_that, _cardToMove, _destination, false, _moveAsVehicle);
                                                    }
                                                });
                                    } else {
                                        _capacitySlotChosen = true;
    ...
    public Effect nextEffect(SwccgGame game) {
        ...
        if (!isAnyCostFailed()) {
            ...
            if (!_useForceCostApplied) {
                ...
                if (!_forFree) {
                    appendCost(new PayEmbarkingCostEffect(_that, getPerformingPlayer(), _cardToMove, _destination, 0));
                    return getNextCost();
                }
            }
    ...
    public boolean wasActionCarriedOut() {
        return _cardMoved && _embarkEffect.wasCarriedOut();
    }
}
