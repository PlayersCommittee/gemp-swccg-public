 ...
 */
public class Card7_057 extends AbstractNormalEffect {
    public Card7_057() {
        super(Side.LIGHT, 4, PlayCardZoneOption.ATTACHED, Title.Coruscant_Celebration, Uniqueness.UNIQUE, ExpansionSet.SPECIAL_EDITION, Rarity.R);
        setLore("Despite Imperial propaganda, the citizens of Coruscant very much wish for the collapse of the Empire.");
    ...
    protected Filter getGameTextValidDeployTargetFilter(SwccgGame game, PhysicalCard self, PlayCardOptionId playCardOptionId, boolean asReact) {
        return Filters.Coruscant_system;
    }

    @Override
    protected boolean checkGameTextDeployRequirements(String playerId, SwccgGame game, PhysicalCard self, PlayCardOptionId playCardOptionId, boolean asReact) {
        return GameConditions.occupies(game, playerId, 2, Filters.Coruscant_battleground_site);
    }
    ...
    protected List<TopLevelGameTextAction> getGameTextTopLevelActions(final String playerId, SwccgGame game, final PhysicalCard self, int gameTextSourceCardId) {
        ...
        if (GameConditions.isOnceDuringYourPhase(game, self, playerId, gameTextSourceCardId, gameTextActionId, Phase.CONTROL)) {
            int numForce = Filters.countTopLocationsOnTable(game, Filters.and(Filters.Coruscant_battleground_site, Filters.occupiesWith(playerId, self, Filters.mayContributeToForceRetrieval)));
            if (numForce > 0) {
