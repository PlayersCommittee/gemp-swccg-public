...
/**
 * An evaluator that returns the number of cards accepted by the specified filter that are in battle, multiplied by -1 (For Card210_013)
 */
public class InBattleEvaluatorNegativeOutput extends BaseEvaluator {
     ...
     */
    public InBattleEvaluatorNegativeOutput(PhysicalCard source, Filterable filters) {
    ...
    public float evaluateExpression(GameState gameState, ModifiersQuerying modifiersQuerying, PhysicalCard self) {
        ...
        return (Filters.countActive(gameState.getGame(), source, Filters.and(filterToUse, Filters.participatingInBattle)) * -1);
    }
}
