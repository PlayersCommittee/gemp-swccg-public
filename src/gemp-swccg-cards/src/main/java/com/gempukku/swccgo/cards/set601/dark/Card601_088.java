...
public class Card601_088 extends AbstractImmediateEffect {
    public Card601_088() {
        super(Side.DARK, 6, PlayCardZoneOption.YOUR_SIDE_OF_TABLE, Title.Imperial_Propaganda, Uniqueness.UNRESTRICTED, ExpansionSet.LEGACY, Rarity.V);
    ...
    protected List<RequiredGameTextTriggerAction> getGameTextRequiredBeforeTriggers(final SwccgGame game, Effect effect, final PhysicalCard self, int gameTextSourceCardId) {
        // Check condition(s)
        if (TriggerConditions.isPlayingCard(game, effect, Filters.and(Filters.not(Icon.LEGACY_BLOCK_4), Filters.Imperial_Propaganda))
...
/*
        ...
        if (TriggerConditions.isTableChanged(game, effectResult)
                && GameConditions.canTargetToCancel(game, self, Filters.and(Filters.not(Icon.LEGACY_BLOCK_4), Filters.Imperial_Propaganda))) {

            final RequiredGameTextTriggerAction action = new RequiredGameTextTriggerAction(self, gameTextSourceCardId, gameTextActionId);
            // Build action using common utility
            CancelCardActionBuilder.buildCancelCardAction(action, Filters.and(Filters.not(Icon.LEGACY_BLOCK_4), Filters.Imperial_Propaganda), Title.Imperial_Propaganda);
            actions.add(action);
        }
