...
/**
 * An effect to modify the hyperspeed of a card until end of the turn.
 */
public class ModifyHyperspeedUntilEndOfTurnEffect extends AbstractSuccessfulEffect {
    private PhysicalCard _cardToModify;
    private float _modifierAmount;

    /**
     * Creates an effect that modifies the hyperspeed of a card until end of the turn.
     * @param action the action performing this effect
     * @param cardToModify the card whose hyperspeed is modified
     * @param modifierAmount the amount of the modifier
     */
    public ModifyHyperspeedUntilEndOfTurnEffect(Action action, PhysicalCard cardToModify, float modifierAmount) {
        super(action);
    ...
    protected void doPlayEffect(SwccgGame game) {
        ...
        if (_modifierAmount < 0)
            actionMsg = "reduces " + GameUtils.getCardLink(_cardToModify) + "'s hyperspeed by " + GuiUtils.formatAsString(-_modifierAmount) + " until end of the turn";
        else
            actionMsg = "adds " + GuiUtils.formatAsString(_modifierAmount) + " to " + GameUtils.getCardLink(_cardToModify) + "'s hyperspeed until end of the turn";
        ...
        modifiersEnvironment.addUntilEndOfTurnModifier(
                new HyperspeedModifier(source, cardFilter, _modifierAmount));
