...
/**
 * An effect to modify the defense value of card until end of the turn.
 */
public class ModifyDefenseValueUntilEndOfTurnEffect extends AbstractSuccessfulEffect {
    private PhysicalCard _cardToModify;
    private float _modifierAmount;

    /**
     * Creates an effect that modifies the defense value of a card until end of the turn.
     * @param action the action performing this effect
     * @param cardToModify the card whose attribute is modified
     * @param modifierAmount the amount of the modifier
     */
    public ModifyDefenseValueUntilEndOfTurnEffect(Action action, PhysicalCard cardToModify, float modifierAmount) {
        super(action);
    ...
    protected void doPlayEffect(SwccgGame game) {
        ...
        if (_modifierAmount < 0)
            actionMsg = "reduces " + GameUtils.getCardLink(_cardToModify) + "'s defense value by " + GuiUtils.formatAsString(-_modifierAmount) + " until end of the turn";
        else
            actionMsg = "adds " + GuiUtils.formatAsString(_modifierAmount) + " to " + GameUtils.getCardLink(_cardToModify) + "'s defense value until end of the turn";
