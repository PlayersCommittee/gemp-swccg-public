 ...
 */
public class PayExtraCostToDeployCardsSimultaneouslyEffect extends AbstractSubActionEffect {
     ...
     * @param attachedCardToDeploy the card to deploy as attached to the other card
     * @param target the deploy target, or null
     * @param forFree true if deploying for free, otherwise false
     */
    public PayExtraCostToDeployCardsSimultaneouslyEffect(Action action, PhysicalCard cardToDeploy, PhysicalCard attachedCardToDeploy, PhysicalCard target, boolean forFree) {
        super(action);
    ...
    public boolean isPlayableInFull(SwccgGame game) {
        ...
        int extraCost = modifiersQuerying.getExtraForceRequiredToDeployToTarget(gameState, _cardToDeploy, _target, null, _action.getActionSource(), _forFree)
                + modifiersQuerying.getExtraForceRequiredToDeployToTarget(gameState, _attachedCardToDeploy, _cardToDeploy, _target, _action.getActionSource(), _forFree);
        if (extraCost == 0)
            return true;
