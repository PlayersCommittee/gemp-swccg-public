...
/**
 * The effect result that is emitted when a starship or vehicle begins to move using sector movement.
 */
public class MovingUsingSectorMovementResult extends EffectResult implements MovingResult {
    ...
    /**
     * Creates an effect result that is emitted when a starship or vehicle begins to move using sector movement.
     ...
     */
    public MovingUsingSectorMovementResult(PhysicalCard cardMoving, String playerId, PhysicalCard movingFrom, PhysicalCard movingTo, boolean asReact, boolean asMoveAway, PreventableCardEffect effect) {
        super(Type.MOVING_USING_SECTOR_MOVEMENT, playerId);
    ...
    public String getText(SwccgGame game) {
        return "Moving " + (_asMoveAway ? "away " : "") + GameUtils.getCardLink(_cardMoving) + " from " + GameUtils.getCardLink(_movingFrom) + " to " + GameUtils.getCardLink(_movingTo) + " using sector movement" + (_asReact ? " as a 'react'" : "");
    }
}
