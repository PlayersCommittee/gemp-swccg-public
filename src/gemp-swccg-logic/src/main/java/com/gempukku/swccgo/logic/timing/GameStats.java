    ...
    public boolean updateGameStats(SwccgGame game) {
        ...
        if (playerOrder != null) {
            for (String player : playerOrder.getAllPlayers()) {
                ...
                playerZoneSizes.put(Zone.FROZEN_PILE, game.getGameState().getFrozenForceSize(player));
                playerZoneSizes.put(Zone.TOP_OF_FROZEN_PILE, game.getGameState().hasFrozenAssetsMarker(player) ? 1 : 0);
