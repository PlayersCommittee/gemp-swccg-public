...
/**
 * The effect result that is emitted when a bomber begins to move to start a Bombing Run.
 */
public class MovingToStartBombingRunResult extends EffectResult implements MovingResult {
    private PhysicalCard _cardMoving;
    private PhysicalCard _movingFrom;
    private PhysicalCard _movingTo;

    /**
     * Creates an effect result that is emitted when a bomber begins to move to start a Bombing Run.
     ...
     */
    public MovingToStartBombingRunResult(PhysicalCard cardMoving, String playerId, PhysicalCard movingFrom, PhysicalCard movingTo) {
        super(Type.MOVING_TO_START_BOMBING_RUN, playerId);
    ...
    public String getText(SwccgGame game) {
        return "Moving " + GameUtils.getCardLink(_cardMoving) + " from " + GameUtils.getCardLink(_movingFrom) + " to " + GameUtils.getCardLink(_movingTo) + " to start a Bombing Run";
    }
}
