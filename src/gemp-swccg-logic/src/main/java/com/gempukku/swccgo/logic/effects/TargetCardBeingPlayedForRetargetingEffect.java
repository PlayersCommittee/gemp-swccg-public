...
/**
 * An abstract class for to target cards being played for re-targeting.
 */
public abstract class TargetCardBeingPlayedForRetargetingEffect extends AbstractTargetCardsEffect {
    private final RespondablePlayingCardEffect _playingCardEffect;

    /**
     * Creates an effect to target card being played for re-targeting.
     * @param action the action performing this effect
     * @param playingCardEffect the playing card effect
     */
    public TargetCardBeingPlayedForRetargetingEffect(Action action, RespondablePlayingCardEffect playingCardEffect) {
        super(action);
    ...
    protected FullEffectResult playEffectReturningResult(final SwccgGame game) {
        ...
        cardTargetedToBeRetargeted(_playingCardEffect.getCard());

        return new FullEffectResult(true);
    }

    /**
     * This method is called when card has been targeted to be re-targeted.
     * @param targetedCard the targeted card
     */
    protected abstract void cardTargetedToBeRetargeted(PhysicalCard targetedCard);
}
