...
/**
 * An effect that carries out the stealing of a single card (and place it in the player's specified card pile).
 */
public class StealOneCardIntoCardPileEffect extends AbstractSubActionEffect implements PreventableCardEffect {
    ...
    private StealOneCardIntoCardPileEffect _that;

    /**
     * Creates an effect that carries out the stealing of a single card (and place it in the player's specified card pile).
     * @param action the action performing this effect
     * @param cardToSteal the card to steal
     * @param cardPile the card pile to place the stolen card
     */
    public StealOneCardIntoCardPileEffect(Action action, PhysicalCard cardToSteal, Zone cardPile) {
        super(action);
                ...
                new PassthruEffect(subAction) {
                    ...
                    protected void doPlayEffect(SwccgGame game) {
                        if (!isEffectOnCardPrevented(_cardToBeStolen)) {
                            ...
                            gameState.sendMessage(_playerId + " steals " + GameUtils.getCardLink(_cardToBeStolen) + fromZoneText + " into " + _cardPile.getHumanReadable() + " using " + GameUtils.getCardLink(_action.getActionSource()));
                            ...
                            gameState.addCardToTopOfZone(_cardToBeStolen, _cardPile, _playerId);
