...
/**
 * An effect to prevent a card from battling until the end of the turn.
 */
public class MayNotBattleUntilEndOfTurnEffect extends AbstractSuccessfulEffect {
    private Collection<PhysicalCard> _targetCards;

    /**
     * Creates an effect that prevents a card from battling until the end of the turn.
     * @param action the action performing this effect
     * @param targetCard the card affected
     */
    public MayNotBattleUntilEndOfTurnEffect(Action action, PhysicalCard targetCard) {
        this(action, Collections.singleton(targetCard));
    }

    /**
     * Creates an effect that prevents cards from battling until the end of the turn.
     * @param action the action performing this effect
     * @param targetCards the cards affected
     */
    public MayNotBattleUntilEndOfTurnEffect(Action action, Collection<PhysicalCard> targetCards) {
        super(action);
    ...
    protected void doPlayEffect(SwccgGame game) {
        ...
        gameState.sendMessage(GameUtils.getAppendedNames(_targetCards) + " may not battle until end of the turn");
