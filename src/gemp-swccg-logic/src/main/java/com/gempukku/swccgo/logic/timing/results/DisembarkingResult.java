...
/**
 * The effect result that is emitted when a card begins to disembark from a card (or from a location).
 */
public class DisembarkingResult extends EffectResult implements MovingResult {
    private PhysicalCard _cardMoving;
    private PhysicalCard _movingFrom;
    private PhysicalCard _movingTo;

    /**
     * Creates an effect result that is emitted when a card begins to disembark from a card (or from a location).
     ...
     */
    public DisembarkingResult(PhysicalCard cardMoving, String playerId, PhysicalCard movingFrom, PhysicalCard movingTo) {
        super(Type.DISEMBARKING, playerId);
    ...
    public String getText(SwccgGame game) {
        return "Disembarking " + GameUtils.getCardLink(_cardMoving) + " from " + GameUtils.getCardLink(_movingFrom) + " to " + GameUtils.getCardLink(_movingTo);
    }
}
