...
/**
 * The effect result that is emitted when a card begins to embark on a card (or to a location).
 */
public class EmbarkingResult extends EffectResult implements MovingResult {
    private PhysicalCard _cardMoving;
    private PhysicalCard _movingFrom;
    private PhysicalCard _movingTo;

    /**
     * Creates an effect result that is emitted when a card begins to embark on a card (or to a location).
     ...
     */
    public EmbarkingResult(PhysicalCard cardMoving, String playerId, PhysicalCard movingFrom, PhysicalCard movingTo) {
        super(Type.EMBARKING, playerId);
    ...
    public String getText(SwccgGame game) {
        return "Embarking " + GameUtils.getCardLink(_cardMoving) + " from " + GameUtils.getCardLink(_movingFrom) + " to " + GameUtils.getCardLink(_movingTo);
    }
}
