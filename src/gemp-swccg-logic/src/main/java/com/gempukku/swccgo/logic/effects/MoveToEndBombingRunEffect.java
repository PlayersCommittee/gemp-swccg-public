...
import com.gempukku.swccgo.logic.timing.results.MovedToEndBombingRunResult;
import com.gempukku.swccgo.logic.timing.results.MovingToEndBombingRunResult;

/**
 * An effect for a bomber to move to end a Bombing Run.
 */
public class MoveToEndBombingRunEffect extends AbstractSubActionEffect {
    ...
    /**
     * Creates an effect for a bomber to move to end a Bombing Run.
     * @param action the action performing this effect
     * @param cardMoved the card to move
     * @param moveTo the location to move to
     */
    public MoveToEndBombingRunEffect(Action action, PhysicalCard cardMoved, PhysicalCard moveTo) {
        super(action);
    ...
    protected SubAction getSubAction(SwccgGame game) {
        ...
        subAction.appendEffect(
                new TriggeringResultEffect(subAction,
                        new MovingToEndBombingRunResult(_cardMoved, _playerId, _movedFrom, _movedTo)));
                ...
                new PassthruEffect(subAction) {
                    ...
                    protected void doPlayEffect(SwccgGame game) {
                        ...
                        if (modifiersQuerying.mayNotMove(gameState, _cardMoved)) {
                            // If card is not able to move, then it is lost instead
                            subAction.appendEffect(
                                    new LoseCardFromTableEffect(_action, _cardMoved));
                            return;
                        }

                        // Move card and send message
                        gameState.moveCardToLocation(_cardMoved, _movedTo);
                        _cardMoved.setMakingBombingRun(false);
                        StringBuilder msgText = new StringBuilder(_playerId).append(" moves ").append(GameUtils.getCardLink(_cardMoved))
                                ...
                                .append(" at end of a Bombing Run");
                        gameState.sendMessage(msgText.toString());
                        _moveCompleted = true;

                        // Emit effect result
                        game.getActionsEnvironment().emitEffectResult(new MovedToEndBombingRunResult(_cardMoved, _playerId, _movedFrom, _movedTo));
                    }
                }
        );
