...
/**
 * The effect result that is emitted when a starship moves between systems without using hyperspeed.
 */
public class MovedWithoutUsingHyperspeedResult extends EffectResult implements MovedResult {
    ...
    /**
     * Creates an effect result that is emitted when a starship moves between systems using hyperspeed.
     ...
     */
    public MovedWithoutUsingHyperspeedResult(PhysicalCard movedCard, String playerId, PhysicalCard moveFrom, PhysicalCard movedTo, boolean asReact) {
        super(Type.MOVED_WITHOUT_USING_HYPERSPEED, playerId);
    ...
    public String getText(SwccgGame game) {
        return "Moved " + GameUtils.getCardLink(_cardMoved) + " from " + GameUtils.getCardLink(_movedFrom) + " to " + GameUtils.getCardLink(_movedTo) + " without using landspeed" + (_asReact ? " as a 'react'" : "");
    }
}
