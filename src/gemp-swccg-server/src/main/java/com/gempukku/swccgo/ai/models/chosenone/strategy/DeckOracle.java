...
public class DeckOracle {
    ...
    public static class DeckCard {
        ...
        // V82.2 mirror — see rando/strategy/DeckOracle.java V82.2 comment.
    ...
    /**
     * V67bg (mirror of Rando, 2026-05-10): SWCCG game-text noun → typed Filter.
     * See Rando DeckOracle.resolveCommonNounToFilter for full rationale.
    ...
    /** V67bg: type-aware reserve check using engine filter semantics. */
    ...
    public boolean hasTargetInZone(Zone zone, String... keywords) {
        ...
        for (DeckCard dc : allCards) {
            ...
            for (String kw : keywords) {
                ...
                // 2. V67s: Strip icon-bracket prefixes like "[Episode I]" — these
                // are metadata, not in titles. "[episode i] lightsaber" → "lightsaber".
                ...
                // 3. V67s: Try last word (often the card type word).
    ...
    public static List<String> parseSourceCardPullTargets(String gameText) {
        ...
        // V82.1 (Steve, 2026-05-16): Drop "deploy" anchor; just match
        // "from Reserve Deck". See rando/strategy/DeckOracle.java V82.1
        ...
        for (java.util.regex.Pattern p : patterns) {
            ...
            while (m.find()) {
                ...
                for (String part : norm.split(",")) {
                    ...
                    // V82.1: peel known verb/article prefixes until stable.
                    String prev;
                    do {
                        prev = t;
                        // Greedy [^,]* — see Rando V82.1 comment.
                    ...
                    // V82.3 (Steve, 2026-05-16): paren/bracket strip — mirror Rando.
    ...
    public PullValidation validatePullFromSourceCard(Zone sourceZone, String sourceCardGameText) {
        ...
        // V82.1 (Steve, 2026-05-16): TYPE-API FALLBACK — mirror Rando.
        // Per Steve's standing rule, search by CardCategory not substring
        // for generic type-words. See rando DeckOracle for full comment.
        for (String t : targets) {
        ...
        // V82.2 (Steve, 2026-05-16): blueprint-predicate fallback — mirror Rando.
        // Handles "neimoidian pilot" (Blockade Flagship), "imperial leader", etc.
        for (String t : targets) {
            ...
            for (DeckCard dc : allCards) {
                ...
                for (String w : words) {
                    if (!wordHasPredicate(w)) continue;
    ...
    /** V82.2: Does this blueprint match a single word via type-API checks? */
