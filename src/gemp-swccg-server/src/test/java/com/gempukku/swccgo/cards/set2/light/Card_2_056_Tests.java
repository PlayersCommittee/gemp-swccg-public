...
public class Card_2_056_Tests {
	...
	/**
	 * Puts your Bothan Spy at the Light starting site with a Stormtrooper.
	 * Then plays Undercover (2_40) on your Bothan Spy so the spy is undercover for real, not via MakeCardGoUndercover.
	...
	public void ThiefUndercoverSpyMayStealDarkWeaponInsteadOfLose() {
		...
		// Deploy Merc Sunlet so play-option 1 actually grants Keyword.THIEF, then play Undercover (2_40)
	...
	public void OnTableDeployCostUsesGlobalAndNonGlobalModifiers() {
		...
		scn.MoveCardsToLocation(mountains, spy, scout);
		scn.SkipToLSTurn(Phase.DEPLOY);
		PlayLightUndercoverOnSpy(scn);
		...
		scn.SkipToLSTurn(Phase.CONTROL);
		scn.PrepareLSDestiny(3); // 3 > modified Scout 2; would fail vs unmodified 3
		assertTrue(scn.LSCardPlayAvailable(sabotage));
		scn.LSPlayCard(sabotage);
		assertTrue(scn.LSHasCardChoiceAvailable(scout));
		scn.LSChooseCard(scout);
		scn.PassAllResponses();
		scn.PassCardLeavingTable();
		assertEquals(Zone.TOP_OF_LOST_PILE, scout.getZone());
	}
